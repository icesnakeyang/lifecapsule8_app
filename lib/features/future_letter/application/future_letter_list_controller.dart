...
final futureLetterListControllerProvider =
    StateNotifierProvider<FutureLetterListController, FutureLetterListState>((
      ref,
    ) {
      return FutureLetterListController(ref);
    });
...
class FutureLetterListController extends StateNotifier<FutureLetterListState> {
  final Ref ref;

  FutureLetterListController(this.ref)
    : super(const FutureLetterListState(loading: true)) {
    refresh();
  }
  ...
  Future<void> refresh() async {
    ...
    try {
      ...
      final items =
          all
              .where((n) => n.kind == NoteKind.futureLetter && !n.isDeleted)
              .toList()
